...
function fetchGroupNames(deliverIds) {
    ...
    deliverIds.forEach(deliverId => {
        promiseArray.push(new PowerPromise(function (resolve, reject) {
...
exports.check = async function (sr, done) {
    ...
    if (sotd) {
        ...
        await Promise.all(fetchGroupNames(sr.getDelivererIDs())).then(function (groupNames) {
            sr.isIGDeliverable = groupNames.some(groupName => {
                return groupName.endsWith('Interest Group');
            });
        });
