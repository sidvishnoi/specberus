...
function buildWanted (groups, sr) {
    ...
    if (isPP2002)
        ...
        if ((config.recTrackStatus && config.noRecTrack )|| sr.isIGDeliverable)
            wanted += noRec;
        else if (isWGNote)
            wanted += "(" + noRec + ')?';
...
exports.check = async function (sr, done) {
    ...
    ;
    var groupName = await fetchGroupName(sr.getDelivererIDs());
    sr.isIGDeliverable = groupName.endsWith('Interest Group');

    // fetchGroupName(sr.getDelivererIDs()).then(function (groupName) {
    //     var isIG = groupName.endsWith('Interest Group');
    // })
